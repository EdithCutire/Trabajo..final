...
function movimientoimg3(){
	
	if(numeroo8 >= 1014) {numeroo8=0}else{numeroo8+=2}

    for (var i = 0; i <= numeroo8; i+=259) {
	  
	    if (numeroo8>=i) {ubicacion8 =i}
    }
    if (numero8>=1014) {numero8=0} else {numero8+=2}

    	for (var a = 0; a <=numero8; a+=10) {
    		if(numero8>=a) {ubicacion81=a}
    	}

// por defecto para borrar el imagen
ctx.clearRect(ubicacion81,0,0,0);
...
//S E G U N DA I M A G E N
//ctx.draw(imagen,x1,y1,recortex,recortey,x2,y2)
ctx.drawImage(sprite3, ubicacion8,100,259,300,ubicacion81,300,250,250);
frame(movimientoimg3);
